...
Clear-Host
Write-Host 'Each Server That Had Its TCP Keep Alive Time Value Changed Will Require A Reboot For The Changes To Take Affect. Press ANY key ...' -ForegroundColor Green
    $x = $host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
                Exit
}
else
{
    Write-Host "PowerShell Version does not meet minimum requirements of at least 3.0, please update to at least PowerShell 3.0 and try again. Press ANY key..." -ForegroundColor Red
    $x = $host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
                Exit
    }
